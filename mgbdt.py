...
class MGBDT:
    def __init__(self, loss='L1Loss', target_lr=0.1, epsilon=0.3, verbose=False):
        """
        Attributes
        ----------
        layers: list
    ...
    def log_metric(self, prefix, X, y, eval_metric):
        ...
        for i in range(pred.shape[0]):
            pr.append(pred[i][0])
        pr = np.array(pr)
        loss = self.calc_loss(pr, y)
        if eval_metric == "mean_squared_error":
            from sklearn.metrics import mean_squared_error
            score = mean_squared_error(y, pr)
        else:
            score = None
        if score is None:
            self.log("{} loss={:.6f}".format(prefix, loss), "green")
        else:
            self.log("{} loss={:.6f}".format(prefix, score), "green")
